...
COVARGS = --cov-config .coveragerc --cov-report html --cov sirtrevor
TCARGS = --timeout=10 -n auto

lint: clean-test ## check style with flake8
	$(PR) flake8 setup.py docs sirtrevor tests
...
test-with-prefix: clean-test ## run tests quickly with the default Python and prefix
	TOX_PREFIX=py36 $(PRP) $(PTARGS) $(TEST)
